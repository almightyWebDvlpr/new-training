...
async function fetchNewVideos() {
    try {
        ...
        const uploadDates = await page.$$eval("div#metadata-line > span.style-scope.ytd-grid-video-renderer", dates => dates.map(date => date.textContent));
        console.log(uploadDates);
        await browser.close();

        // Check if uploadDates includes "часа", "час", or "часов"
        const hasHourUpload = uploadDates.some(date => date.includes("часа") || date.includes("час") || date.includes("часов"));
        if (hasHourUpload) {
            ...
            if (videoLinks.length > 0) {
                ...
                await bot.sendMessage(chatId, `У нашому YouTube каналі, RGS UKRAINE додане нове відео - ${videoLinks[0]}`);
                console.log("Message sent for new video uploaded within the last hour.");
            } else {
                console.log("No video links available.");
            }
        } else {
            // Otherwise, process uploadDates normally
            const videosWithDates = videoLinks.map((link, index) => ({ link, uploadDate: uploadDates[index] }));
...
const emailContent = {
    ...
    subject: 'Інформація про інформаційний телеграм бот від телемедичної платформи Rehabilitation Gaming System',
    html: `
    <h1>Добрий день,</h1>
...
bot.on("text", async (msg) => {
    ...
    if (dayOfWeek !== 0 && dayOfWeek !== 6 && !messageSentToday) {
        await bot.sendMessage(msg.chat.id, 'Вітаємо і нагадуємо якщо, у Вас виникають питання або труднощі з використання платформи звертайтеся до служби підримки за номером телефону: +380634390602 чи електронною поштою - rgs.info.ua@ukr.net або за потребою створіть запит на навчання! Дякуємо і гарного дня.');
...
async function saveToGoogleSheets(msg, data) {
    ...
    //   await bot.sendMessage(msg.chat.id, applicationMessage);
      await bot.sendMessage(msg.chat.id, "Дякуємо. Ваш запит успішно надіслано. Ми зв'яжемося з Вами найближчим часом.");
    } catch (error) {
      console.error("Error appending data to Google Sheets:", error);
      await bot.sendMessage(msg.chat.id, "Error saving application to Google Sheets.");
    }
  }
